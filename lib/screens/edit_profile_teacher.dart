...
class _ViewTeacherProfilePageState extends State<ViewTeacherProfilePage> {
  ...
  Map<String, dynamic> _validateProfileData(Map<String, dynamic>? data) {
    ...
    return {
      ...
      'timezone': data['timezone']?.toString().trim() ?? 'Asia/Kuala_Lumpur',
  ...
  Future<void> fetchStatistics() async {
    ...
    try {
      ...
      // Validate email
      if (!_isValidEmail(teacherEmail?.toString())) {
        debugPrint('Invalid email in profile: $teacherEmail');
        setState(() => loadingStats = false);
        return;
      }
      ...
      // Validate tasks data
      if (!_validateStatisticsData(createdTasks)) {
        debugPrint('Invalid tasks data structure');
      }

      totalAssignmentsCreated = createdTasks.length;

      // 2. Total Students (unique students assigned tasks) with validation
      final uniqueStudents = <String>{};
      for (var task in createdTasks) {
        final studentEmail = task['student_email']?.toString();
        if (studentEmail != null && _isValidEmail(studentEmail)) {
          uniqueStudents.add(studentEmail);
        }
      }
      totalStudents = uniqueStudents.length;

      // 3. Average Class Performance (avg points of completed tasks) with validation
      final completedTasks = createdTasks
          .where((task) =>
              task['status'] == 'completed' && task['points'] != null)
          .toList();
      ...
      if (completedTasks.isNotEmpty) {
        ...
        if (points.isNotEmpty) {
          ...
          averageClassPerformance = averageClassPerformance.clamp(0.0, 100.0);
        }
      }

      // 4. Active This Month (students who submitted this month) with validation
      ...
      for (var task in createdTasks) {
        if (task['completed_timestamp'] != null) {
          try {
            ...
            if (completedDate.isAfter(now.add(const Duration(days: 1)))) {
              debugPrint('Invalid completed date (future): $completedDate');
              continue;
            }
            ...
            if (completedDate.isAfter(firstDayOfMonth)) {
              final studentEmail = task['student_email']?.toString();
              if (studentEmail != null && _isValidEmail(studentEmail)) {
                activeStudentsSet.add(studentEmail);
              }
            }
          } catch (e) {
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              const Text("No teacher profile found"),
              const SizedBox(height: 16),
              ElevatedButton(
                onPressed: fetchProfile,
                child: const Text('Cuba Semula'),
